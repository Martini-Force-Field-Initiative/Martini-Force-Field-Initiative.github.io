...
In this tutorial, we will prepare different Martini 3 structure bias models for the protein kinase casein kinase delta 1 (CK1δ) \[22\]. In Sections [I.I](Tut1.qmd) and [I.II](Tut2.qmd), we will focus solely on the structured region of this protein (residues 1-292), while the IDR (residues 293-415) will be covered in [Section I.III](Tut3.qmd). We use the experimental crystal structure with PDB code 4JJR as a starting structure \[22\].

4JJR is crystallised as a dimer, and also has several missing residues. It is important to validate initial input structures for simulation at the point before proceeding. For expediency, we provide a valid input structure for a monomer ([model_01.pdb](https://cgmartini-library.s3.ca-central-1.amazonaws.com/0_Tutorials/m3_tutorials/ProteinsI/model_01.pdb)). Molecular Dynamics Parameters (`.mdp`) files for Gromacs for Martini simulations are found [here](https://cgmartini.nl/docs/downloads/example-input-files/md-parameters.html). Otherwise, following the steps below will show you how to ensure that you begin a simulation with a valid protein structure.

<details>
<summary>Generate a valid PDB file from scratch</summary>

Start by downloading the structure with the PDB code 4JJR from the RCSB-PDB using:
...
If you inspect `martini_v3.0.0.itp` at this point, you should now see these lines have been inserted like so:
