...
As [before](../Small_Molecule_Parametrization/index.qmd#comparison-to-experimental-results-further-refinements-and-final-considerations) it is imperative that any newly parameterised molecule for the Martini force field is validated against as many references as possible. The log P and SASA are


#### 7.1 Molecular volume and shape
<hr>

To automate the creation of a vdwradii.dat for a CG molecule, a small Python script, `make_cg_sasa_file.py` is provided with this tutorial to automate the process. Otherwise, the steps and scripts from the previous tutorial can be followed again.
