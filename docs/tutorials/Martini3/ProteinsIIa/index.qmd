---
title: "Proteins - Part IIa: Transmembrane peptides"
format: html
---

<hr>

In case of issues, please contact [luis.borges\@ens-lyon.fr](mailto:%20luis.borges@ens-lyon.fr).
...
-   [`3. Insert your protein in a membrane environment`](#proteins-in-membrane-environment)
    ...
    -   [`3.2. Pre-built approach`](#pre-built-approach)
        -   [`Option 1: Insane`](#pre-built-approach)
        -   [`Option 2: COBY`](#pre-built-approach)
-   [`4. Going Further`](#GoingFurther)
    -   [`4.1. Studying the impact of lipid composition`](#insane)
    -   [`4.2. Studying peptide dimerization`](#coby)
-   [`5. Tools and scripts used in this tutorial`](#tools-and-scripts-used-in-this-tutorial)
...
Adding this bias generates additional files, `virtual_sites_atomtypes.itp` and `virtual_sites_nonbond_params.itp` which include the definition of the virtual sites. Have a look at these files and familiarize yourself with their format. 

To use them, you must modify your `martini_v3.0.0.itp` file to include their content conditionally. This can be done by inserting the following lines between the `[ atomtypes ]` and `[ nonbond_params ]` sections (line 858).
...
Finally, activate the bias by adding the following line to the top of your `.top` file:
...
`system.top` should look something like this:
...
You can easily find the number of waters and lipids in your system using grep to correctly finalize your `.top` file: 
...
The previous command line will set up a complete system, containing a squared POPC bilayer of $7×7 nm^2$, with the KALP peptide inserted and centered in this bilayer. The whole system will be solvated in standard coarse-grained water. A small concentration of NaCl was added only to neuralize the +4 charge of KALP. 

`insane.py` provides a prototype system.top file. However, you should always double-check that all `#include` statements match the components in your system and point to the correct `.itp` files.

More on the `insane.py` tool can be found in separate tutorials, notably setting up a complex bilayer ([lipid tutorial part II](../LipidsII/index.qmd)).
...
COBY.COBY(
    ...
    protein = "file:M3_proteins_tutorial_part2/KALP/self-assembly/kalp.pdb moleculetypes:molecule_0",
    ...
    itp_input = [
        "include:M3_proteins_tutorial_part2/KALP/self-assembly/martini_v3.0.0.itp",
        "include:M3_proteins_tutorial_part2/KALP/self-assembly/martini_v3.0.0_ions_v1.itp",
        "include:M3_proteins_tutorial_part2/KALP/self-assembly/martini_v3.0.0_phospholipids_v1.itp",
        "include:M3_proteins_tutorial_part2/KALP/self-assembly/martini_v3.0.0_solvents_v1.itp",
        "include:M3_proteins_tutorial_part2/KALP/self-assembly/molecule_0.itp",
    ],
...
This command creates a POPC bilayer of size 7 × 7 nm² with the KALP peptide centered in the membrane. The system is then solvated with coarse-grained water and neutralized with a small concentration of NaCl.

COBY automatically generates a system.top file and includes relevant #include statements. However, as always, you should carefully check that all entries are consistent with your system and that required .itp files are present and correctly referenced.
...
### 4.1. Studying the impact of lipid composition. {#GoingFurther1}
...
### 4.2. Studying peptide dimerization. {#GoingFurther2}
