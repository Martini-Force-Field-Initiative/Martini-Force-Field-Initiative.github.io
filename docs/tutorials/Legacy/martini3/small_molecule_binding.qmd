...
**Note1**: This tutorial, now based on the Martini 3 open-beta version ([v.3.0.b.3.2](https://martini-library.s3.us-east-2.amazonaws.com/ff_parameters/martini3_beta/martini_3.0.b.3.2.tar)), will be updated as soon as the final parameters of Martini 3 are available (the paper is under review right now).
...
You can look for more details about the insane.py script [here](../../../downloads/tools/proteins-and-bilayers.qmd#insane). The output file (`CG.gro`) contains `NA+`, `CL-`, and `W` residues. Check how many and add specify it in the `CG.top` file.
