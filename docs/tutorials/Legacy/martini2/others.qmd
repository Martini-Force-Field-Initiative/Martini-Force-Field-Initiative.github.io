...
If you want to solvate a new system with polarizable water, follow the steps in the lipid or protein Martini tutorials. Instead of a normal waterbox and `.mdp/.itp` files use the waterbox containing polarizable water and polarizable Martini `.mdp` and `.itp` files (available to download [here](../../../downloads/index.qmd)). Note that minimizing a polarizable Martini system requires some tweaking, as described below. We encourage you to get all downloads yourself, but if you insist on taking a short-cut, you can download an archive with just the basic files [here](https://martini-library.s3.us-east-2.amazonaws.com/legacy_material/martini2/martini_Pwater-Refined.tar) and with worked example output [here](https://martini-library.s3.us-east-2.amazonaws.com/legacy_material/martini2/martini_Pwater-Refined-WORKED.tar).

If you have an existing system with normal Martini water and want to change to polarizable water you may use the python script `[triple-w.py](https://martini-library.s3.us-east-2.amazonaws.com/legacy_material/martini2/triple-w.py)`. For this example we convert [dppc_bilayer.gro/dppc_bilayer.top available here](../../../downloads/example-applications/lipid-membranes.qmd#dppc-bilayer).
...
The structure (`.gro`) files after each equilibration step and after the $1 \mu s$ simulation can be downloaded [here](https://martini-library.s3.us-east-2.amazonaws.com/legacy_material/martini2/martini-maker-dry-vesicle-1us.tar).
