...
const createContactFunction = new lambda.Function(this, 'CreateContactLambda', {
    ...
    contactTable.grantWriteData(createContactFunction); // allow the createOrder lambda function to write to the order table
    contactQueue.grantSendMessages(createContactFunction); // allow the createOrder lambda function to send messages to the order processing queue
