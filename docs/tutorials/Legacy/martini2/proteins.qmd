...
Two first steps are done using the publicly available `martinize.py` script, of which the latest version can be downloaded [from Github](http://cgmartini.nl/index.php/tools2/proteins-and-bilayers/204-martinize). However, if you want to be sure you use the same version as the files provided as an example, download [this version of martinize.py](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/soluble-protein/ubiquitin/martini/martinize.py). The last step can be done with the tools available in the gromacs package and/or with ad hoc scripts. In this part of the tutorial, some basic knowledge of `gromacs` commands is assumed and not all commands will be given explicitly. Please refer to the previous tutorials (basic tutorial on Martini lipids available [here](http://cgmartini.nl/index.php/tutorials-general-introduction-gmx5/bilayers-gmx5)) and/or the gromacs manual.
...
[Download an example of all the worked files of this module](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/soluble-protein/ubiquitin.tgz)
...
3. If everything went well, the script generated three files: a coarse-grained structure (`.pdb`; cf. Fig. 1), a master topology file (`.top`), and a protein topology file (`.itp`). In order to run a simulation you need two more: [the Martini topology file](../../downloads/force-field-parameters/particle-definitions.qmd) (`martini_v2.x.itp`, if you specified version 2.2, you'll want the corresponding file) and a run parameter file (`.mdp`). You can get [examples](../../downloads/force-field-parameters/input-parameters.qmd) from the Martini website or from the protein tutorial package. Don't forget to adapt the settings where needed! 
...
[Download an example of all the worked files of this module, with gromacs version 5.0.7](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial/soluble-protein/hiv-1_protease.tgz)

[Download an example of all the worked files of this module with gromacs 2016.3](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/soluble-protein/hiv-1_protease.tgz)
...
* the protein is charged (each monomer has charge +2). It is recommended to add at least neutralizing counter ions; on top of that, you can add ions to reflect some ionic strength of the solution. NOTE that you'll need to get hold of a Martini topology file that specifies the [ion topologies](../../downloads/force-field-parameters/ions.qmd). 
...
<hr>
[Download an example of all the worked files of this module](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial/membrane-protein/kalp.tgz)

[Download an example of all the worked files of this module using GROMACS version 2016.3](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/membrane-protein/kalp.tgz)

In the last module of this protein tutorial, we will increase the complexity of our system by embedding the protein in a lipid bilayer (the tutorial on the lipidome, lipid self-assembly and bilayer analysis can be followed [here](./lipids_and_lipidome.qmd)). We propose here to have a look at the tilt and the dimerization of KALP peptides embedded in DPPC membrane. The coarse-grained structure ([kalp.gro](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/membrane-protein/kalp/self-assembly/monomer-DPPC/kalp.gro)) and topology ([kalp.itp](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/membrane-protein/kalp/self-assembly/monomer-DPPC/kalp.itp)) of KALP can just be downloaded by clicking on the links; no need to repeat the steps to martinize the peptide. We start by solvating the protein directly with the membrane environment so that it ends up inserted. Different ways to perform this step possible; a non-exhaustive list, ordered by decreasing complexity, is presented below (note that this tutorial will focus only on the third case):

- A bilayer self-assembly dynamics, as presented in the [first module of the lipid tutorial](http://cgmartini.nl/index.php/tutorials-general-introduction-gmx5/bilayers-gmx5), will result in the self-insertion of the KALP inside the bilayer (Fig. 5). This method is not followed in this tutorial but you will also find the respective files in the downloaded bundle.
...
- The use of the script `insane.py`, downloadable [here](http://cgmartini.nl/index.php/insane), allows the setting up of bilayers of arbitrary composition around our protein.
...
2. Proceed as before and start a production run. To remind you, this involves (1) editing the system.top file to reflect the version of Martini you want to use (the [KALP topology provided](http://cgmartini.nl/images/stories/tutorial/2017/protein-tutorial-v2016.3/membrane-protein/kalp/self-assembly/monomer-DPPC/kalp.itp) is version 2.1) and providing include statements for the topology files; (2) [downloading](../../downloads/force-field-parameters/input-parameters.qmd) or copying definitions of Martini version you want and the DPPC lipid topology file from [the lipidome](../../downloads/force-field-parameters/lipids.qmd); (3) using the correct names of the molecules involved; (4) downloading or copying set-up `.mdp` files for minimization, equilibration, and production runs and if necessary, editing them (bilayer simulations are best done using semi-isotropic pressure coupling and you may want to separately couple different groups to the thermostat(s)); (5) running the minimization and equilibration runs.  
...
- `martinize.py` (downloadable [here](http://cgmartini.nl/index.php/tools2/proteins-and-bilayers/204-martinize))
- `insane.py` (downloadable [here](http://cgmartini.nl/index.php/insane))
