...
<script>
  ...
  function scrollToElement(selector) {
    ...
    smoothScroll({toElement: targetElement, duration: 1500});
  }
</script>
...
<script>
  ...
  for (let i = 0; i < 35; i++) {
    ...
    doodle.style.animationDelay = `${i * -0.6}s`;
    
    // Randomize background color and size
    const colors = ['blue', 'green', 'yellow', 'white'];
    const color = colors[Math.floor(Math.random() * colors.length)];
    const size = Math.random() * 2 + 1;
    doodle.style.background = `radial-gradient(${color} 10%, transparent 40%)`;
