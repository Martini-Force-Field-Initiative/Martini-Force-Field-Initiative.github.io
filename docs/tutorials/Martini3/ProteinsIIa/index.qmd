...
**The files required for this tutorial (including worked files) can be downloaded from [here](https://cgmartini-library.s3.ca-central-1.amazonaws.com/0_Tutorials/m3_tutorials/ProteinsII/KALP_ProteinsII_Tutorial.zip).**
...
Another way to build a Martini membrane system is with [**TS2CG 2.0**](https://github.com/weria-pezeshkian/TS2CG-v2.0) <sup>[\[10,11\]](#references)</sup>, a tool for constructing coarse-grained membranes either from triangulated surfaces or from analytical shape definitions. The TS2CG 2.0 workflow is organized around three components: **PLM**, **PCG**, and **SOL**. For a simple flat bilayer, the relevant steps are **PCG** for membrane construction and **SOL** for solvation and ion addition. 

We will build a **7 × 7 × 9 nm³** Martini 3 system containing:

-   a **POPC bilayer**,
-   a single **KALP peptide** embedded in the membrane,
-   water and salt,
-   and GROMACS-ready output files for simulation.

TS2CG can be installed with `pip` directly from [GitHub](https://github.com/weria-pezeshkian/TS2CG-v2.0).

``` bash
pip install git+https://github.com/weria-pezeshkian/TS2CG-v2.0
```

After installation, it is useful to inspect the command-line help:
...
##### Building a flat POPC bilayer with an inserted peptide

TS2CG uses an input description file. For the present system, the input must define three things:

1.  a **flat membrane shape** inside a **7 × 7 × 9 nm³ box**,
2.  a membrane composition of **100% POPC**,
3.  and a single peptide entry corresponding to **KALP**.

A draft `input.str` file for this kind of setup is shown below:
...
First, it is specified which lipid types we want to build in which domain and for which leaflet. Here, we only have domain 0 and we want to populate it exclusively with POPC in both monolayers. We supply an area per lipid for building as well.

Then, we want to add the protein. To add the protein, we include the gro file at the top and then specify in the protein list. Note that the name of the protein needs to match the name in the gro file. We can directly add multiple copies of the protein, we adjust the first value such that one protein will be placed. The other entrys describe the displacement of the protein. In most cases, only the last one is used to adjust the height of the protein in the membrane. As we require a .gro file, run `gmx editconf -f kalp.pdb -o kalp.gro` and set the name in the first line of the .gro file.

Finally, we want to use an analytical shape to build the membrane, arbitrary shapes are possible from triangualted surfaces through the PLM module of TS2CG. We add the Shape Data for a flat membrane in a box size of 7x7x9 nm. More analytical shapes can be found [here](https://github.com/weria-pezeshkian/TS2CG-v2.0/wiki/Tutorial-5) and how to build arbitrary membrane shapes is explained [here](https://github.com/weria-pezeshkian/TS2CG-v2.0/wiki/Tutorial-1).

With the membrane description in place, run **PCG** to construct the membrane system:

``` bash
TS2CG PCG -str input.str -defout system -function analytical_shape
```

This step builds the membrane and writes a structure and topology for the unsolvated system. It is a sensible approach to minimize the system in vacuum before solvating.

##### Solvating the system

Once the membrane-protein system has been generated, use **SOL** to add solvent.

The command is given below. It takes a water template and populates the box. A cutoff can be specified how close water beads can be placed to an existing structure:

``` bash
TS2CG SOL -in system.gro -o solvated.gro -tem water.gro
```

TS2CG SOL will provide a info.txt with the information of how many beads of what type were placed. Add these numbers to your topology file.

At this stage, your main files are:

-   `solvated.gro` — solvated coordinates,
-   `system.top` — topology

##### Topology and Martini 3 files

Before simulation, the include statements for a simulation in GROMACS need to be added to the topology file. In total, check that

-   all required Martini 3 `.itp` files are present,
-   the peptide molecule name matches between coordinate and topology files,
-   the number of lipids, solvent molecules, and ions is correct,
-   and any `#include` statements in `system.top` point to the correct files.
...
\[10\] Pezeshkian, W., König, M., Wassenaar, T. A., & Marrink, S. J. (2020). Backmapping triangulated surfaces to coarse-grained membrane models. Nature communications, 11(1), 2296. https://doi.org/10.1038/s41467-020-16094-y

\[11\] Schuhmann, F., Stevens, J. A., Rahmani, N., Lindahl, I., Brown, C. M., Brasnett, C., ... & Pezeshkian, W. (2025). TS2CG as a membrane builder. Journal of Chemical Theory and Computation, 21(18), 9136-9146. https://doi.org/10.1021/acs.jctc.5c00833 