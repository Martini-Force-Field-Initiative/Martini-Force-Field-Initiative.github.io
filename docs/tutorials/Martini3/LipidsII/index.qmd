...
``` bash
pip install --force-reinstall --no-deps --user simopt git+https://github.com/Tsjerk/Insane.git
```

At this point, this version of `Insane` will be available under the `.local/` directory tree in your home. If `$HOME/.local/bin` is not already in your path, add it:
``` bash
...
This DBPC:DLPC mixture separates in a couple hundred ns at a temperature of 290 K (Fig. 1C,D,E). If you don’t want to wait that long for your simulation, a sample run is provided for you under `phase_sep.xtc` (if you're not using the 'worked' tutorial files, you can get it from [here](https://cgmartini-library.s3.ca-central-1.amazonaws.com/0_Tutorials/m3_tutorials/LipidsII/phase_sep.xtc)). If you do run the simulation, take the time to revisit `Insane`'s `lipids.dat` and understand its structure.
