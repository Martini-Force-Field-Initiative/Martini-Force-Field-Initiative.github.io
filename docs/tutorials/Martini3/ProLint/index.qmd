...
::: callout-note
`ProLint2` relies on `MDAnalysis` to read input data files, which makes it compatible with all the most popular MD engines, including GROMACS, CHARMM, NAMD, LAMMPS, and OpenMM.
:::
...
::: callout-warning
This will add the LIPX lipid type to the database group. Please notice we are using the residue name "LIPX" here as an example of a custom lipid type, but you can use any residue name that is relevant to your system. As LIPX is not part of the GIRK system, it will not add anything in this case.
:::
...
``` output
defaultdict(<function prolint2.metrics.exact_contacts.ExactContacts.pooled_results.<locals>.<lambda>()>,
            {np.int64(14): defaultdict(list,
                         {'POPE': [np.float64(4.0),
                           np.float64(4.0),
                           np.float64(8.0),
                           np.float64(4.0),
                           ...
                           np.float64(4.0),
...
                         {'POPS': [np.float64(4.0), np.float64(4.0)]}),
             np.int64(5): defaultdict(list, {'POPS': [np.float64(4.0)]}),
             np.int64(793): defaultdict(list, {'POPS': [np.float64(4.0)]}),
             np.int64(287): defaultdict(list, {'POPE': [np.float64(4.0)]}),
             np.int64(306): defaultdict(list, {'POPE': [np.float64(4.0)]})})
```
...
``` output
defaultdict(<function prolint2.computers.contacts.SerialContacts.__init__.<locals>.<lambda>()>,
            {np.int64(14): defaultdict(list,
                         {np.int64(2527): [0,
                           15,
                           20,
                          ...
                          np.int64(2800): [2,
                           ...
                           21,
...
             np.int64(1089): defaultdict(list, {np.int64(2670): [1181, 1197]}),
             np.int64(5): defaultdict(list, {np.int64(2740): [1185]}),
             np.int64(793): defaultdict(list, {np.int64(2696): [1204]}),
             np.int64(287): defaultdict(list, {np.int64(2558): [1236]}),
             np.int64(306): defaultdict(list, {np.int64(2508): [1236]})})
```
...
MPD.create_plot()
```
![Exploratory Point Distribution Plot: Columns show the different lipid types, and the rows represent different metrics.](fig4.png)

It becomes clear from this analysis that interactions with POPE are not significant, while there are certain specific interactions with CHOL and POPS that are worth exploring in more detail. Different ProLint2 Plotters can be used to focus the analysis on certain lipid types and metrics. For this tutorial,  we have decided to use normalized lipid density profiles and logo plots to analyze the interactions with POPS lipids.
...
![Surface representation of the GIRK protein. In red are Lys361 and Lys367, and in blue is Val354.](fig7.jpeg)

The specificity of this interaction is anticipated due to the positively charged lysine side chain, which can engage in electrostatic interactions with the negatively charged headgroup of POPS. To further investigate this interaction, we can use an Interaction Heatmap Plotter to visualize the contact patter at the atom-atom level of detail. For these analyses, we must first identify the POPS molecule that most frequently interact with one of these residues, let's choose Lys461. We can get this information by using the get_lipid_contact_durations() function, which provides a dataframe listing the lipid molecule IDs sorted by their total contact frequencies.
