...
Download the base files for this tutorial [here](https://martini-library.s3.us-east-2.amazonaws.com/legacy_material/martini2/PEG_parametrization.tar). Download and unpack the archive (tar -xzvf PEG_parametrization.tgz unpacks to the directory tree PEG_parametrization).
...
The solvation free energies of our PEG9 molecule have been determined by thermodynamic integration (TI) at the fine-grain level. You can access that data [here](https://martini-library.s3.us-east-2.amazonaws.com/legacy_material/martini2/TI_tutorial.tar). The archive unpacks (tar -xzvf TI_tutorial.tgz) to a directory tree TI_tutorial. This archive contains the atomistic data and a `CG` directory with `H2O` and `Melt` subdirectories ready for you to populate with your systems.
