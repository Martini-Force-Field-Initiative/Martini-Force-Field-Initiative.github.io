...
_**Figure 1: Showcase models.** a) The bentopy logo, packed with bentopy. Those interested in how this model was constructed may enjoy this [thread on the making of this model](https://hachyderm.io/@ma3ke/114081904829438250). b) Mitochondrial cytoplasm model, showing distinct compartments with compartment-specific molecular components._
...
2. Neutralize the system net charge due to the net positive charge of each lysozyme monomer  by substituting additional ions.
...
bentopy-mask structures/double_membrane.gro outer_mask.npz \
        --no-interactive --labels -1
# Interactive mask creation for inner compartment: label -2
bentopy-mask structures/double_membrane.gro inner_mask.npz \
...
outer-compartment from "outer_mask.npz"                                        # <1>
inner-compartment from "inner_mask.npz"
membrane-neighborhood within 4 of membrane
inner-close-to-membrane combines membrane-neighborhood and inner-compartment   # <2>

[ segments ]
LYZ:lyz 200 from "structures/lysozyme.pdb" in outer-compartment                # <3>
UBQ:ubq 100 from "structures/ubiquitin.pdb" in inner-close-to-membrane         # <4>
```

1. Added multiple compartments (inner and outer).
...
3. Place lysozyme only in the outer compartment.
4. Place ubiquitin only in the inner compartment _and_ close to the membrane.
