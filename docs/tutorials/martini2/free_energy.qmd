...
All files needed for this part of the tutorial can be found in the FreeEnergyTechniques folder in [FreeEnergyTechniques.zip](http://cgmartini.nl/images/stories/tutorial/2017/FreeEnergyTechniques_updated_2022.zip). Each part of this tutorial has its own directory which itself contains a folder called files_DoItYourself. There you can find the necessary files to follow the tutorial. For the second and the third part (umbrella sampling and thermodynamic integration) we also provide a worked version in the folder files_worked.

Open a terminal (ctrl-a) and download the .zip file to your desktop (or some other convenient place):

``` bash
cd ~/Desktop
wget http://www.cgmartini.nl/images/stories/tutorial/2017/FreeEnergyTechniques_updated_2022.zip
```
...
More details of the theory behind the US technique can be found in reference [[3](http://cgmartini.nl/index.php/tutorials-general-introduction-gmx5/partitioning-techniques#Ref3)]. Hereafter, we describe step by step how to perform the US simulation. Now, let us start the hands-on section.
